...
export function AboutBusinessForm({ versionId, initialValues, aiPrompt }: { versionId: string; initialValues: Values; aiPrompt: string }) {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); queueSave(latest.current, true); }}>
    ...
    <div className={styles.field}><div className={styles.labelRow}><label htmlFor="founderStory">Meet the founder <span className={styles.optional}>(optional)</span></label><span>{values.founderStory.length}/2,000</span></div><p className={styles.hint}>Tell visitors about yourself, your background, how you started, and why you started this business. This is an opportunity to build a personal connection with your potential customers.</p><textarea id="founderStory" rows={8} maxLength={2000} value={values.founderStory} onChange={(event) => update("founderStory", event.target.value)} placeholder="Share your story—what led you to start this business and what drives you today." /></div>
