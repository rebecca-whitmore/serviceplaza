...
export function BasicInformationForm({ versionId, categories, serviceTags, initialValues }: {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); queueSave(latest.current, true); }}>
    ...
    <div className={styles.saveBar}><p aria-live="polite" className={hasError ? styles.error : ""}>{status}</p><div className={styles.saveActions}><button className={styles.saveOnly} type="button" onClick={() => queueSave(latest.current)}>Save now</button><button type="submit">Save and continue</button></div></div>
  </form>;
}
