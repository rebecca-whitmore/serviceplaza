...
export function ContactDetailsForm({ versionId, initialValues }: { versionId: string; initialValues: Values }) {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); queueSave(latest.current, true); }}>
    <aside className={styles.infoBox}><strong>Private or public?</strong><p>Your sign-in email remains private. The details below are for your listing. Your email and telephone number will only be displayed when you explicitly choose to show them.</p></aside>
    ...
    <div className={styles.contactGroup}><div className={styles.field}><label htmlFor="publicEmail">Listing email address <span className={styles.optional}>(optional)</span></label><input id="publicEmail" type="email" autoComplete="email" maxLength={320} value={values.publicEmail} onChange={(event) => update({ publicEmail: event.target.value, showPublicEmail: event.target.value ? values.showPublicEmail : false })} /></div>
      <label className={styles.permissionChoice}><input type="checkbox" checked={values.showPublicEmail} disabled={!values.publicEmail.trim()} onChange={(event) => update({ showPublicEmail: event.target.checked })} /><span><strong>Display this email publicly</strong><small>If left unticked, visitors will not see this email address.</small></span></label>
    </div>

    <div className={styles.contactGroup}><div className={styles.field}><label htmlFor="publicPhone">Listing telephone number <span className={styles.optional}>(optional)</span></label><input id="publicPhone" type="tel" autoComplete="tel" maxLength={40} value={values.publicPhone} onChange={(event) => update({ publicPhone: event.target.value, showPublicPhone: event.target.value ? values.showPublicPhone : false })} /></div>
      <label className={styles.permissionChoice}><input type="checkbox" checked={values.showPublicPhone} disabled={!values.publicPhone.trim()} onChange={(event) => update({ showPublicPhone: event.target.checked })} /><span><strong>Display this telephone number publicly</strong><small>If left unticked, visitors will not see this telephone number.</small></span></label>
    </div>
