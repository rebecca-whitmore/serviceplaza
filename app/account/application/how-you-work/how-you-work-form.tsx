...
export function HowYouWorkForm({ versionId, initialValues }: { versionId: string; initialValues: Values }) {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); queueSave(latest.current, true); }}>
    ...
    <fieldset className={styles.fieldset}><legend>Where is your business based?</legend><p className={styles.hint}>We do not need your street address. Your county or region is required even if you work entirely online or serve customers across the UK.</p><div className={styles.locationBox}><div className={styles.field}><div className={styles.labelRow}><label htmlFor="baseTownCity">Base town or city <span className={styles.optional}>(optional)</span></label><span>{values.baseTownCity.length}/120</span></div><p className={styles.hint}>This is kept private for now.</p><input id="baseTownCity" autoComplete="address-level2" maxLength={120} value={values.baseTownCity} onChange={(event) => update({ baseTownCity: event.target.value })}/></div><div className={styles.field}><div className={styles.labelRow}><label htmlFor="ukRegion">County or region</label><span>{values.ukRegion.length}/120</span></div><input id="ukRegion" autoComplete="address-level1" maxLength={120} value={values.ukRegion} onChange={(event) => update({ ukRegion: event.target.value })}/></div></div><label className={styles.permissionChoice}><input type="checkbox" checked={values.showBaseLocation} onChange={(event) => update({ showBaseLocation: event.target.checked })}/><span><strong>Show my county or region on my public listing</strong><small>Sharing your general location can help visitors discover nearby businesses, even if you also work online or serve customers across the UK.</small></span></label></fieldset>
