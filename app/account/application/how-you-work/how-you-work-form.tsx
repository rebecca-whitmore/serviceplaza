...
export function HowYouWorkForm({ versionId, initialValues }: { versionId: string; initialValues: Values }) {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); queueSave(latest.current, true); }}>
    ...
    {values.offersInPerson ? <fieldset className={styles.fieldset}><legend>How do customers work with you in person?</legend><p className={styles.hint}>This helps us show your business to customers whose location you genuinely cover.</p><div className={styles.optionGrid}>{([
      ...
      {["customers_visit", "both"].includes(values.inPersonMode) ? <div className={styles.locationBox}><div className={styles.field}><div className={styles.labelRow}><label htmlFor="publicVisitAddress">Full address customers should visit</label><span>{values.publicVisitAddress.length}/500</span></div><p className={styles.hint}>Enter the complete public address, including the building or street, town or city, county and postcode.</p><textarea id="publicVisitAddress" rows={5} maxLength={500} value={values.publicVisitAddress} onChange={(event) => update({ publicVisitAddress: event.target.value, publicVisitAddressConfirmed: false })} placeholder={"Business or building name\nStreet address\nTown or city\nCounty\nPostcode"}/></div><label className={styles.checkChoice}><input type="checkbox" checked={values.publicVisitAddressConfirmed} onChange={(event) => update({ publicVisitAddressConfirmed: event.target.checked })}/><span><strong>I confirm that this full address may be displayed publicly</strong><small>Customers will see this address on your Service Plaza listing so they know where to visit.</small></span></label></div> : null}
