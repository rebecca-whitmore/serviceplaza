...
export function HowYouWorkForm({ versionId, initialValues }: { versionId: string; initialValues: Values }) {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); queueSave(latest.current, true); }}>
    ...
    <div className={styles.locationBox}><div className={styles.field}><label htmlFor="businessPostcode">Business base postcode</label><p className={styles.postcodeNote}><strong>Note for online only businesses:</strong><br/>Your postcode is kept private. Visitors will only see a broader area.<br/>Being discoverable as a nearby independent business can help extend your reach.</p><input id="businessPostcode" autoComplete="postal-code" maxLength={8} value={values.businessPostcode} onChange={(event) => update({ businessPostcode: event.target.value.toUpperCase() })} placeholder="For example, CH1 2AB"/></div></div>
    ...
    {values.offersInPerson ? <fieldset className={styles.fieldset}><legend>How do customers work with you in person?</legend><p className={styles.hint}>This helps us show your business to customers whose location you genuinely cover.</p><div className={styles.optionGrid}>{([
      ...
      {["customers_visit", "both"].includes(values.inPersonMode) ? <div className={styles.locationBox}><div className={styles.field}><div className={styles.labelRow}><label htmlFor="publicVisitAddress">Full address customers should visit</label><span>{values.publicVisitAddress.length}/500</span></div><p className={styles.hint}>Enter the complete public address, including the building or street, town or city, county and postcode. This will be listed publicly so that customers know how to find you.</p><textarea id="publicVisitAddress" rows={5} maxLength={500} value={values.publicVisitAddress} onChange={(event) => update({ publicVisitAddress: event.target.value })} placeholder={"Business or building name\nStreet address\nTown or city\nCounty\nPostcode"}/></div></div> : null}
      {values.inPersonMode && values.inPersonMode !== "customers_visit" ? <div className={styles.locationBox}><div className={styles.field}><label htmlFor="travelRadiusMiles">How far do you travel?</label><select id="travelRadiusMiles" value={values.inPersonNationwide ? "nationwide" : values.travelRadiusMiles} onChange={(event) => event.target.value === "nationwide" ? update({ inPersonNationwide: true }) : update({ inPersonNationwide: false, travelRadiusMiles: Number(event.target.value) })}>{COVERAGE_MILES.map((miles) => <option value={miles} key={miles}>{miles} {miles === 1 ? "mile" : "miles"}</option>)}<option value="nationwide">Nationwide</option></select>{values.inPersonNationwide ? <p className={styles.hint}>Choose Nationwide only if you genuinely travel to customers throughout the UK.</p> : null}</div></div> : null}
    </fieldset> : null}
