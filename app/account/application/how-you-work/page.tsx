...
export default async function HowYouWorkPage() {
  ...
  const { data: draft } = await supabase.from("listing_versions").select("id, is_uk_based, show_base_location, offers_online, offers_in_person, serves_local, serves_uk_wide, base_town_city, uk_region").eq("listing_id", listing.id).eq("status", "draft").maybeSingle();
