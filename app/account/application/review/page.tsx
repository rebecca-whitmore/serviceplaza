...
export default async function ReviewPage() {
  ...
  const customersVisit = draft.offers_in_person && ["customers_visit", "both"].includes(draft.in_person_mode ?? "");
  const workComplete = Boolean(draft.is_uk_based && draft.business_postcode?.trim() && (draft.offers_online || draft.offers_in_person) && (!draft.offers_in_person || draft.in_person_mode) && (!customersVisit || (draft.public_visit_address?.trim() && draft.public_visit_address_confirmed)));
  ...
  return <><header className={styles.header}><div><h1>Review &amp; submit</h1></div></header><p className={styles.intro}>Check how your application comes together. Use any Edit link to make changes before sending it to Service Plaza for review.</p>
    <div className={styles.reviewGrid}>
      ...
      <ReviewSection title="How & where you work" href="/account/application/how-you-work" complete={workComplete}><Items items={[["UK business base", draft.is_uk_based ? "Verified by postcode" : null], ["How you work", [draft.offers_online && "Online", draft.offers_in_person && "In person"].filter(Boolean).join(", ")], ["Business base postcode", draft.business_postcode], ["Public area", [draft.base_town_city, draft.uk_region].filter(Boolean).join(", ")], ["In-person arrangement", draft.in_person_mode === "travels_to_customer" ? "I travel to customers" : draft.in_person_mode === "customers_visit" ? "Customers visit me" : draft.in_person_mode === "both" ? "Both" : null], ["Public customer address", customersVisit && draft.public_visit_address_confirmed ? draft.public_visit_address : null], ["Travel coverage", draft.in_person_nationwide ? "Nationwide" : draft.in_person_mode !== "customers_visit" && draft.travel_radius_miles ? `${draft.travel_radius_miles} miles` : "Not applicable"]]} /></ReviewSection>
