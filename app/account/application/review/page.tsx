...
export default async function ReviewPage() {
  ...
  const workComplete = Boolean(draft.is_uk_based && (draft.offers_online || draft.offers_in_person) && (!draft.offers_in_person || draft.base_town_city?.trim() || draft.uk_region?.trim()));
  ...
  return <><header className={styles.header}><div><h1>Review &amp; submit</h1></div></header><p className={styles.intro}>Check how your application comes together. Use any Edit link to make changes before sending it to Service Plaza for review.</p>
    <div className={styles.reviewGrid}>
      ...
      <ReviewSection title="How & where you work" href="/account/application/how-you-work" complete={workComplete}><Items items={[["UK-based business", draft.is_uk_based ? "Confirmed" : null], ["How you work", [draft.offers_online && "Online", draft.offers_in_person && "In person"].filter(Boolean).join(", ")], ["Public in-person location", draft.offers_in_person ? [draft.base_town_city, draft.uk_region].filter(Boolean).join(", ") : "Not applicable"]]} /></ReviewSection>
