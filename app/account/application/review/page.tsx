...
export default async function ReviewPage() {
  ...
  const perkComplete = !draft.has_plaza_perk || Boolean(draft.perk_title?.trim() && draft.perk_description?.trim() && draft.perk_redemption?.trim()); const standOutComplete = Boolean(image && perkComplete);
  ...
  return <><header className={styles.header}><div><h1>Review &amp; submit</h1></div></header><p className={styles.intro}>Check how your application comes together. Use any Edit link to make changes before sending it to Service Plaza for review.</p>
    <div className={styles.reviewGrid}>
      ...
      <ReviewSection title="How & where you work" href="/account/application/how-you-work" complete={workComplete}><Items items={[["How you work", [draft.offers_online && "Online", draft.offers_in_person && "In person"].filter(Boolean).join(", ")], ["Area served", [draft.serves_local && "Local area", draft.serves_uk_wide && "Across the UK"].filter(Boolean).join(", ")], ["Local base", draft.serves_local ? [draft.base_town_city, draft.uk_region].filter(Boolean).join(", ") : null]]} /></ReviewSection>
      <ReviewSection title="Make your listing stand out" href="/account/application/stand-out" complete={standOutComplete}><div className={styles.reviewImage}>{signed?.signedUrl ? <img src={signed.signedUrl} alt={image?.alt_text || "Business image preview"} /> : null}<Items items={[["Image", image?.original_filename], ["Displayed publicly", image ? (image.display_publicly ? "Yes" : "No — standard Service Plaza image will be used") : null], ["Plaza Perk", draft.has_plaza_perk ? draft.perk_title : "Not added"], ["Perk details", draft.has_plaza_perk ? draft.perk_description : null], ["How to claim", draft.has_plaza_perk ? draft.perk_redemption : null], ["Conditions", draft.perk_conditions], ["Expiry", draft.perk_expires_on]]} /></div></ReviewSection>
    </div>
