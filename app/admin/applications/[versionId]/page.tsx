...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ versionId: string }> }) {
  ...
  const { data: listingVersions } = await supabase.from("listing_versions").select("id, version_number").eq("listing_id", listing.id);
  const listingVersionIds = (listingVersions ?? []).map((item) => item.id);
  ...
  return <><Link className={styles.back} href="/admin">← Review queue</Link><header className={styles.header}><div><h1>{version.business_name || "Unnamed business"}</h1>{version.status === "pending" ? <Link className={styles.back} style={{ marginTop: ".75rem", marginBottom: 0, padding: ".55rem .8rem", border: "1px solid #62778c", borderRadius: ".55rem" }} href={`/admin/applications/${version.id}/edit`}>Edit submitted application</Link> : null}</div><div className={styles.meta}><strong>{version.status.replaceAll("_", " ")}</strong><span>Submitted {formatDate(version.submitted_at)}</span><span>Version {version.version_number}</span></div></header>
    <div className={styles.grid}><div className={styles.sections}>
      ...
      <Section title="About the business"><List items={[["Short summary", version.short_summary], ["Full description", version.full_description], ["Meet the founder", version.founder_story]]} /></Section>
      <Section title="How & where they work"><List items={[["UK business base", version.is_uk_based ? "Verified by postcode" : "Not verified"], ["Delivery", [version.offers_online && "Online", version.offers_in_person && "In person"].filter(Boolean).join(", ")], ["Business base postcode", version.business_postcode], ["Public base area", [version.base_town_city, version.uk_region].filter(Boolean).join(", ")], ["In-person arrangement", version.in_person_mode?.replaceAll("_", " ")], ["Public customer address", version.public_visit_address_confirmed ? version.public_visit_address : "Not confirmed for publication"], ["Travel coverage", version.in_person_nationwide ? "Nationwide" : version.travel_radius_miles ? `${version.travel_radius_miles} miles` : "Not applicable"]]} /></Section>
      ...
      <Section title="Review history">{events?.length ? <ul className={styles.history}>{events.map((event, index) => <li key={`${event.created_at}-${index}`}><strong>{event.event_type.replaceAll("_", " ")}</strong><span>Version {listingVersions?.find((item) => item.id === event.listing_version_id)?.version_number ?? "?"} · {formatDate(event.created_at)}</span>{event.applicant_message ? <p>Applicant message: {event.applicant_message}</p> : null}{event.private_admin_note ? <p>Private note: {event.private_admin_note}</p> : null}</li>)}</ul> : <span className={styles.empty}>No review events recorded.</span>}</Section>
