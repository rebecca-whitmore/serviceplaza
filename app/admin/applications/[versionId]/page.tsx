...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ versionId: string }> }) {
  ...
  return <><Link className={styles.back} href="/admin">← Review queue</Link><header className={styles.header}><div><h1>{version.business_name || "Unnamed business"}</h1></div><div className={styles.meta}><strong>{version.status.replaceAll("_", " ")}</strong><span>Submitted {formatDate(version.submitted_at)}</span><span>Version {version.version_number}</span></div></header>
    <div className={styles.grid}><div className={styles.sections}>
      ...
      <Section title="How & where they work"><List items={[["UK-based business", version.is_uk_based ? "Confirmed" : "Not confirmed"], ["Business base", [version.base_town_city, version.uk_region].filter(Boolean).join(", ")], ["Public location", version.show_base_location ? `${version.uk_region} shown publicly` : "Not shown publicly"], ["Delivery", [version.offers_online && "Online", version.offers_in_person && "In person"].filter(Boolean).join(", ")], ["Area served", [version.serves_local && "Local area", version.serves_uk_wide && "Across the UK"].filter(Boolean).join(", ")]]} /></Section>
