...
export default async function AdminListingDetailPage({ params, searchParams }: { params: Promise<{ listingId: string }>; searchParams: Promise<{ notice?: string; error?: string }> }) {
  ...
  const publicImageUrl = publishedImagePath ? supabase.storage.from("listing-images-public").getPublicUrl(publishedImagePath).data.publicUrl : null;
  ...
  return <><Link className={styles.back} href="/admin/listings">← Back to listings</Link>
    <header className={styles.header}><div><p>Approved listing · Version {version.version_number}</p><h1>{version.business_name}</h1><div className={styles.headerActions}><span className={listing.publication_status === "published" ? styles.published : styles.hidden}>{listing.publication_status === "published" ? "Published" : "Hidden"}</span>{listing.publication_status === "published" && version.is_uk_based ? <Link className={styles.publicLink} href={`/business/${listing.slug}`} target="_blank">View public listing ↗</Link> : null}</div></div><div className={styles.owner}><strong>{business?.contact_name ?? "Owner not provided"}</strong><span>{business?.contact_email ?? "Email not provided"}</span><span>Published {formatDate(listing.published_at)}</span></div></header>
    {query.notice ? <p className={styles.success}>{query.notice === "hidden" ? "The listing is now hidden from the public directory." : query.notice === "edited" ? "Your changes have been published as a new approved version." : "The listing has been restored to the public directory."}</p> : null}
    ...
    <div className={styles.columns}><section className={styles.content}><section className={styles.card}><h2>Public presentation</h2>{publicImageUrl ? <img className={styles.image} src={publicImageUrl} alt={image?.alt_text ?? "Business listing image"}/> : <div className={styles.placeholder}>Standard placeholder image</div>}<h3>Short summary</h3><p>{text(version.short_summary)}</p><h3>Full description</h3><p className={styles.preline}>{text(version.full_description)}</p></section>
      ...
      <section className={styles.card}><h2>Contact and delivery</h2><dl><div><dt>UK-based business</dt><dd>{version.is_uk_based ? "Confirmed" : "Not yet confirmed"}</dd></div><div><dt>Business base</dt><dd>{[version.base_town_city, version.uk_region].filter(Boolean).join(", ") || "Not provided"} · {version.show_base_location ? "shown publicly" : "kept private"}</dd></div><div><dt>Public contact</dt><dd>{text(version.public_contact_name)}</dd></div><div><dt>Email</dt><dd>{version.show_public_email ? text(version.public_email) : "Not displayed"}</dd></div><div><dt>Telephone</dt><dd>{version.show_public_phone ? text(version.public_phone) : "Not displayed"}</dd></div><div><dt>Website</dt><dd>{text(version.website_url)}</dd></div><div><dt>Social links</dt><dd>{socialLinks.length ? socialLinks.map(([name, url]) => `${name}: ${url}`).join(" · ") : "None"}</dd></div><div><dt>Works</dt><dd>{[version.offers_online && "Online", version.offers_in_person && "In person"].filter(Boolean).join(" and ") || "Not provided"}</dd></div><div><dt>Serves</dt><dd>{[version.serves_local && "Local area", version.serves_uk_wide && "UK-wide"].filter(Boolean).join(" and ") || "Not provided"}</dd></div></dl></section>
      ...
      <aside className={styles.sidebar}><section className={styles.card}><h2>Administrator edits</h2>{activeUpdate ? <p>An owner update is already in progress. Resolve it before making an administrator correction.</p> : <><p>Create a new approved version while preserving this version in the history.</p><Link className={styles.editLink} href={`/admin/listings/${listing.id}/edit`}>Edit listing</Link></>}</section><section className={styles.card}><h2>Visibility controls</h2>{listing.publication_status === "published" ? <form action={setListingVisibility}><input type="hidden" name="listingId" value={listing.id}/><input type="hidden" name="makeVisible" value="false"/><label htmlFor="reason">Reason for hiding</label><textarea id="reason" name="reason" rows={4} maxLength={2000} required/><p>This takes effect immediately but does not delete the listing.</p><button className={styles.danger} type="submit">Hide listing</button></form> : <form action={setListingVisibility}><input type="hidden" name="listingId" value={listing.id}/><input type="hidden" name="makeVisible" value="true"/><label htmlFor="reason">Restoration note <span>(optional)</span></label><textarea id="reason" name="reason" rows={3} maxLength={2000}/><button type="submit">Restore listing</button></form>}</section>
      <section className={styles.card}><h2>Management history</h2>{history?.length ? <ol className={styles.history}>{history.map((event) => <li key={event.id}><strong>{event.action === "hidden" ? "Listing hidden" : event.action === "edited" ? "Administrator edit published" : "Listing restored"}</strong><span>{formatDate(event.created_at)}</span>{event.reason ? <p>{event.reason}</p> : null}</li>)}</ol> : <p>No management changes recorded.</p>}</section></aside></div>
  </>;
}
