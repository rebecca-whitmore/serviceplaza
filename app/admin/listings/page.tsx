...
export default async function AdminListingsPage() {
  ...
  const [{ data: versions }, { data: businesses }, { data: updateVersions }] = await Promise.all([
    versionIds.length ? supabase.from("listing_versions").select("id, business_name, version_number").in("id", versionIds) : Promise.resolve({ data: [] }),
    businessIds.length ? supabase.from("businesses").select("id, contact_name, contact_email").in("id", businessIds) : Promise.resolve({ data: [] }),
    listingIds.length ? supabase.from("listing_versions").select("listing_id, status, version_number").in("listing_id", listingIds).in("status", ["draft", "pending", "changes_requested"]) : Promise.resolve({ data: [] }),
  ]);

  const rows = (listings ?? []).map((listing) => ({
    ...listing,
    version: versions?.find((version) => version.id === listing.current_published_version_id),
    business: businesses?.find((business) => business.id === listing.business_id),
    update: updateVersions?.filter((version) => version.listing_id === listing.id).sort((a, b) => b.version_number - a.version_number)[0],
  }));
