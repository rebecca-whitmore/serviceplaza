...
export default async function AdminPage() {
  ...
  return <><header className={styles.header}><div><p className={styles.eyebrow}>Private administration</p><h1>Review queue</h1></div><p>Applications submitted by business users appear here for private Service Plaza review.</p></header>
    ...
    <section className={styles.queue}><header className={styles.queueHeader}><h2>Applications</h2><span>Oldest submissions first</span></header>{rows.length ? <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Business</th><th>Applicant</th><th>Submitted</th><th>Status</th><th>Action</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><div className={styles.businessName}><strong>{row.business_name || "Unnamed business"}</strong><span>{row.id}</span></div></td><td><div className={styles.businessName}><strong>{row.contactName ?? "Not provided"}</strong><span>{row.contactEmail ?? "No email"}</span></div></td><td>{formatDate(row.submitted_at ?? row.updated_at)}</td><td><span className={styles.status}>{row.status === "pending" ? "Awaiting review" : "Changes requested"}</span></td><td><Link href={`/admin/applications/${row.id}`}>Review</Link></td></tr>)}</tbody></table></div> : <div className={styles.empty}><strong>No applications are awaiting review.</strong><p>Your first submitted test application will appear here.</p></div>}</section>
    <p className={styles.nextNote}>Open an application to inspect its complete private submission. Decision controls will be added after the review view is verified.</p>
  </>;
}
