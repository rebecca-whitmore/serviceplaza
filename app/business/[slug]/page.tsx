...
export default async function PublicListingPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ website?: string }> }) {
  ...
  return <div className={styles.page}><header className={styles.siteHeader}><Link className={styles.brand} href="/"><span>SP</span><strong>Service Plaza</strong></Link><nav aria-label="Public navigation"><Link href="/">Home</Link><Link href="/login">Business account</Link></nav></header>
    <main><div className={styles.breadcrumb}><Link href="/">Service Plaza</Link><span aria-hidden="true">/</span><span>{primaryCategory?.name ?? "UK businesses"}</span></div>
      ...
      <section className={styles.hero}><div className={styles.visual}>{imageUrl ? <img src={imageUrl} alt={`${listing.business_name} business image`}/> : <div className={styles.placeholder}><span>SP</span><small>Service Plaza</small></div>}{listing.has_plaza_perk ? <span className={styles.perkBadge}>Plaza Perk</span> : null}</div><div className={styles.heroCopy}><div className={styles.eyebrows}><span>UK-based business</span>{primaryCategory ? <span>{primaryCategory.name}</span> : null}</div><h1>{listing.business_name}</h1><p className={styles.summary}>{listing.short_summary}</p><div className={styles.facts}>{workModes.map((item) => <span key={String(item)}>{item}</span>)}{serviceAreas.map((item) => <span key={String(item)}>{item}</span>)}{location ? <span>Based in {location}</span> : null}</div>{listing.website_url ? <a className={styles.primaryAction} href={`/go/${listing.slug}/website`} target="_blank" rel="noopener noreferrer">Visit website <span aria-hidden="true">↗</span></a> : null}{listing.has_plaza_perk ? <a className={styles.perkJump} href="#plaza-perk">View their Plaza Perk</a> : null}</div></section>
