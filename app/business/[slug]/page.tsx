...
export default async function PublicListingPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ website?: string; contact?: string }> }) {
  ...
  const inPersonDetail = listing.in_person_mode === "travels_to_customer"
    ? listing.in_person_nationwide ? "Travels to customers nationwide" : `Travels to customers within ${listing.travel_radius_miles ?? 30} miles`
    : listing.in_person_mode === "customers_visit" ? "Customers visit this business"
  ...
  return <div className={styles.page}><header className={styles.siteHeader}><Link className={styles.brand} href="/"><span>SP</span><strong>Service Plaza</strong></Link><nav aria-label="Public navigation"><Link href="/">Home</Link></nav></header>
    <main><div className={styles.breadcrumb}><Link href="/">Service Plaza</Link><span aria-hidden="true">/</span><span>{primaryCategory?.name ?? "UK businesses"}</span></div>
      ...
      <section className={styles.hero}><div className={styles.visual}>{imageUrl ? <img src={imageUrl} alt={`${listing.business_name} business image`}/> : <div className={styles.placeholder}><span>SP</span><small>Service Plaza</small></div>}{listing.has_plaza_perk ? <span className={styles.perkBadge}>Plaza Perk</span> : null}</div><div className={styles.heroCopy}><div className={styles.eyebrows}>{primaryCategory ? <span>{primaryCategory.name}</span> : null}</div><h1>{listing.business_name}</h1><p className={styles.summary}>{listing.short_summary}</p><div className={styles.facts}>{workModes.map((item) => <span key={String(item)}>{item}</span>)}{location ? <span>Based in {location}</span> : null}</div>{listing.website_url ? <a className={styles.primaryAction} href={`/go/${listing.slug}/website`} target="_blank" rel="noopener noreferrer">Visit website <span aria-hidden="true">↗</span></a> : null}{listing.has_plaza_perk ? <a className={styles.perkJump} href="#plaza-perk">View their Plaza Perk</a> : null}</div></section>

      <div className={styles.layout}><div className={styles.mainColumn}><BusinessTabs businessName={listing.business_name} fullDescription={listing.full_description} founderStory={listing.founder_story} />
        {(services.length || serviceTags.length) ? <section className={styles.section}><p className={styles.kicker}>What we offer</p><h2>Services and specialisms</h2>{services.length ? <ul className={styles.services}>{services.map((service) => <li key={service.name}>{service.name}</li>)}</ul> : null}{serviceTags.length ? <div className={styles.tags}>{serviceTags.map((tag) => <span key={tag.name}>{tag.name}</span>)}</div> : null}</section> : null}
        <section className={styles.section}><p className={styles.kicker}>Working together</p><h2>How and where we work</h2><div className={styles.detailGrid}><div><strong>Appointments and delivery</strong><p>{workModes.join(" and ") || "Contact the business for details."}{inPersonDetail ? ` · ${inPersonDetail}` : ""}</p></div>{location ? <div><strong>{listing.offers_in_person ? "In-person area" : "Business base"}</strong><p>{location}</p></div> : null}{listing.public_visit_address ? <div><strong>Address for customer visits</strong><p className={styles.publicAddress}>{listing.public_visit_address}</p></div> : null}</div></section>
      ...
      </div><aside className={styles.sidebar}><section className={styles.contactCard}><p className={styles.kicker}>Get in touch</p><h2>Contact {listing.business_name}</h2>{listing.public_contact_name ? <p>Contact <strong>{listing.public_contact_name}</strong></p> : null}<EnquiryForm slug={listing.slug} businessName={listing.business_name ?? "this business"}/><div className={styles.alternativeContacts}><span>Other ways to connect</span>{listing.website_url ? <a className={styles.primaryAction} href={`/go/${listing.slug}/website`} target="_blank" rel="noopener noreferrer">Visit website <span aria-hidden="true">↗</span></a> : null}{listing.public_email ? <a href={`/go/${listing.slug}/email`}>Email {listing.public_contact_name ?? "the business"}</a> : null}{listing.public_phone ? <a href={`/go/${listing.slug}/phone`}>{listing.public_phone}</a> : null}{socials.length ? <div className={styles.socials}>{socials.map(([name]) => <a key={name} href={`/go/${listing.slug}/${name}`} target="_blank" rel="noopener noreferrer">{label(name)} <span aria-hidden="true">↗</span></a>)}</div> : null}</div></section>
    ...
    </main><footer><Link className={styles.brand} href="/"><span>SP</span><strong>Service Plaza</strong></Link><p>Independent UK-based service businesses, brought together.</p><nav className={styles.footerLinks} aria-label="Footer navigation"><Link href="/about">About Service Plaza</Link><Link href="/terms-and-conditions">Terms &amp; Conditions</Link><Link href="/cookie-policy">Cookie Policy</Link><Link href="/privacy-policy">Privacy Policy</Link><a href="mailto:admin@serviceplaza.co.uk">admin@serviceplaza.co.uk</a><Link href="/login">Business account</Link></nav></footer></div>;
}
