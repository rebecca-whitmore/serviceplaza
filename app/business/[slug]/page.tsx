  ...
  return <div className={styles.page}><header className={styles.siteHeader}><Link className={styles.brand} href="/"><span>SP</span><strong>Service Plaza</strong></Link><nav aria-label="Public navigation"><Link href="/">Home</Link></nav></header>
    <main><div className={styles.breadcrumb}><Link href="/">Service Plaza</Link><span aria-hidden="true">/</span><span>{primaryCategory?.name ?? "UK businesses"}</span></div>
      ...
      <div className={styles.layout}><div className={styles.mainColumn}><BusinessTabs businessName={listing.business_name} fullDescription={listing.full_description} founderStory={listing.founder_story} />
        ...
        <section className={styles.section}><p className={styles.kicker}>Working together</p><h2>How and where we work</h2><div className={styles.detailGrid}><div><strong>Appointments and delivery</strong><p>{workModes.join(" and ") || "Contact the business for details."}</p></div>{location ? <div><strong>In-person location</strong><p>{location}</p></div> : null}</div></section>
