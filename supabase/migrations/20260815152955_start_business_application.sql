-- One authenticated BU can create or resume one editable draft for their
-- single MVP business/listing. The function owns all stable-record creation so
-- the browser never receives direct insert privileges for those tables.

create unique index listing_versions_one_draft
on public.listing_versions (listing_id)
where status = 'draft';

create function public.start_application(contact_name text)
returns table (
  business_id uuid,
  listing_id uuid,
...
revoke all on function public.start_application(text) from public;
grant execute on function public.start_application(text) to authenticated;
