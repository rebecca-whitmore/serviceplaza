-- Listings without an uploaded image use the standard public placeholder.
...
begin
  ...
  if not exists (select 1 from public.listing_service_tags where listing_version_id = target.id) and not exists (select 1 from public.listing_services where listing_version_id = target.id) then raise exception 'At least one service is required'; end if;
