...
begin
  ...
  if target.offers_in_person and trim(coalesce(target.base_town_city, '')) = '' and trim(coalesce(target.uk_region, '')) = '' then raise exception 'In-person location is required'; end if;
