-- Preserve the published founder story when a business owner starts an edit.
-- The original function predates founder_story and therefore omitted it from
-- the new draft, even though the rest of the approved description was copied.
create or replace function public.start_listing_edit()
...
begin
  ...
  insert into public.listing_versions(
    ...
    business_name, short_summary, full_description, founder_story, public_contact_name,
    ...
    source.business_name, source.short_summary, source.full_description, source.founder_story, source.public_contact_name,
...
-- Repair open owner drafts created by the faulty function. Only blank stories
-- are filled, and only from the exact version that the draft supersedes.
update public.listing_versions draft
set founder_story = source.founder_story
from public.listing_versions source
where draft.status = 'draft'
  and draft.supersedes_version_id = source.id
  and nullif(trim(draft.founder_story), '') is null
  and nullif(trim(source.founder_story), '') is not null;
